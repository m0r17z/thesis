...
def preamble(i):
    ...
    module = os.path.join(train_folder, 'cnn_2c2h_regression.py')
...
def load_data(pars):
   data = h5.File('/nthome/maugust/thesis/train_val_test_binning_real_cnn_regression.hdf5','r')
...
def new_trainer(pars, data):
    ...
    m = Cnn(input_size, pars['n_hidden_conv'], pars['n_hidden_full'], output_size,
            pars['hidden_conv_transfers'], pars['hidden_full_transfers'], 'identity',
            loss='squared',image_height=im_height,image_width=im_width,n_image_channel=n_channels,pool_size=pars['pool_size'],filter_shapes=pars['filter_shapes'],
