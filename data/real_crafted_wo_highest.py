...
def generate_real_dataset_crafted():
    ...
    for ind, qpoint_list in enumerate(qpoint_lists):
        ...
        for area in np.arange(12):
            ...
            vec[area*11+10] = area_highest_pow[area]
            vec[area*11+11] = area_pow_means[area]
    ...
    f = h5.File("./crafted_real_int_wo_highest.hdf5", "w")
    f.create_dataset('data_set/data_set', (len(good_samples),144), dtype='f')
    ...
    generate_train_val_test_set("./crafted_real_int_wo_highest.hdf5", "train_val_test_crafted_real_int_wo_highest.hdf5")
